...
/*   Updated: 2024/12/06 03:49:33 by bsengeze         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int load_all_textures(t_data *data) {
  // Load North texture
  data->textures.tex_n = load_texture(data->game.mlx, data->textures.path_n[1]);
  if (!data->textures.tex_n)
    return (1);

  // Load South texture
  data->textures.tex_s = load_texture(data->game.mlx, data->textures.path_s[1]);
  if (!data->textures.tex_s)
    return (1);

  // Load East texture
  data->textures.tex_e = load_texture(data->game.mlx, data->textures.path_e[1]);
  if (!data->textures.tex_e)
    return (1);

  // Load West texture
  data->textures.tex_w = load_texture(data->game.mlx, data->textures.path_w[1]);
